...
# -------------------------
def players_helper(text: str = "Numbers on profile are weighted key metrics for each title scored into percentiles (0–99) vs players in same position in league. Tap to open Individual Metrics which are individual metric percentile scores. Listed in order of Minutes Played"):
...
def draw_team_radar(labels, A_r, B_r, ticks, metrics, headerA, headerB):
    ...
    fig = plt.figure(figsize=(12.8, 7.6), dpi=240)
    fig.patch.set_facecolor(PAGE_BG)
...
# ---- MAIN ----
try:
    teams_df = _load_team_df(TEAM_CSV_PATH)
except Exception as e:
    st.error(f"Could not load {TEAM_CSV_PATH}: {e}")
