...
LEAGUE_TEXT = "Super League"  # purely display

# Header ratings (manual, as you asked)
OVERALL = 88
ATT_HDR = 66
MID_HDR = 77
DEF_HDR = 79
...
# --- Position chip colors (kept from your pro layout) ---
_POS_COLORS = {
...
# --- Country -> twemoji flag (online CDN, lightweight) ---
TWEMOJI_SPECIAL = {
...
# ============================================================
# ROLE DEFINITIONS (FROM YOUR MESSAGE)
...
def pos_group(pos: str) -> str:
    ...
    if p.startswith(("LCMF", "RCMF", "LDMF", "RDMF", "DMF", "CMF")):
        return "CM"
    # “Attackers” in your definition (wingers/AMF)
...
def compute_role_scores_for_row(row: pd.Series) -> dict[str, int]:
    ...
    if g == "CB":
        roles = {k: weighted_role_score(row, w) for k, w in CB_ROLES.items()}
        return roles
    if g == "FB":
        roles = {k: weighted_role_score(row, w) for k, w in FB_ROLES.items()}
        return roles
    if g == "ATT":
        roles = {k: weighted_role_score(row, w) for k, w in ATT_ROLES.items()}
        return roles
    if g == "CF":
        roles = {k: weighted_role_score(row, w) for k, w in CF_ROLES.items()}
        return roles
    if g == "CM":
        roles = {k: weighted_role_score(row, w) for k, w in CM_ROLES.items()}
        # For Central Midfielders: keep TOP 3 only
...
def detect_minutes_col(df: pd.DataFrame) -> str:
    candidates = ["Minutes", "Min", "mins", "minutes", "Minutes played", "Minutes Played"]
    for c in candidates:
        if c in df.columns:
            return c
...
# ============================================================
# HEADER (badge left, info right) — components.html avoids markdown escaping
# ============================================================
...
<style>
  ...
  .crest-img {{ width: 190px; height: 190px; object-fit: contain; display:block; }}
  ...
  .league-text {{ font-size:28px; font-weight:700; color:#d2d2d4; line-height:1; }}
  ...
  .pill {{
    ...
    font-size:26px; font-weight:900; color:#111; border:1px solid rgba(0,0,0,.35);
...
# ============================================================
# LOAD + FILTER + SORT BY MINUTES (DESC)
# ============================================================
if not os.path.exists(CSV_PATH):
    st.error(f"CSV not found at: {CSV_PATH}. Put it in your repo at that path.")
    st.stop()
...
# Sort by minutes descending (your rule)
df = df.sort_values(mins_col, ascending=False).reset_index(drop=True)


# ============================================================
# PRO LAYOUT (sorted by minutes already)
# ============================================================
st.markdown(
    """
    <style>
    ...
    .pro-card{
      position:relative; width:min(620px,98%); display:grid; grid-template-columns:96px 1fr 64px; gap:12px; align-items:start;
    ...
    .pill{ padding:2px 6px; min-width:36px; border-radius:6px; font-weight:800; font-size:18px; line-height:1; color:#0b0d12; text-align:center; display:inline-block; }
    ...
    .postext{ font-weight:700; font-size:14.5px; letter-spacing:.2px; margin-right:10px; }
    ...
    .teamline{ color:#dbe3ff; font-size:14px; font-weight:650; margin-top:6.5px; letter-spacing:.05px; opacity:.95; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    </style>
    """,
    unsafe_allow_html=True
)

DEFAULT_AVATAR = "https://i.redd.it/43axcjdu59nd1.jpeg"  # keep simple; you can replace with local later
...
def _contract_year(row: pd.Series) -> str:
    ...
    if not col:
        return "—"
    cy = pd.to_datetime(row.get(col), errors="coerce")
...
# Main list heading (optional; you can remove)
...
for i, row in df.iterrows():
    ...
    roles: dict = row.get("RoleScores", {}) if isinstance(row.get("RoleScores", {}), dict) else {}
    # Display roles in descending score
    ...
    ) if roles_sorted else "<div class='row'><span class='chip'>No role scores (missing percentile columns)</span></div>"
