...
# =========================
_POS_COLORS = {
    "CF": "#6EA8FF", "LWF": "#6EA8FF", "LW": "#6EA8FF", "LAMF": "#6EA8FF",
    "RW": "#6EA8FF", "RWF": "#6EA8FF", "RAMF": "#6EA8FF",
    "AMF": "#7FE28A", "LCMF": "#5FD37A", "RCMF": "#5FD37A",
    "RDMF": "#31B56B", "LDMF": "#31B56B", "DMF": "#31B56B", "CMF": "#5FD37A",
...
# Expanded country-name -> ISO/flag code map (big)
COUNTRY_TO_CC = {
    # UK home nations
    ...
    # Europe
    "ireland": "ie", "republic of ireland": "ie", "spain": "es", "france": "fr",
    "germany": "de", "italy": "it", "portugal": "pt", "netherlands": "nl",
    ...
    "slovenia": "si", "croatia": "hr", "serbia": "rs", "bosnia and herzegovina": "ba",
    "bosnia": "ba", "montenegro": "me", "kosovo": "xk", "albania": "al",
    "greece": "gr", "hungary": "hu", "romania": "ro", "bulgaria": "bg",
    "russia": "ru", "ukraine": "ua", "georgia": "ge", "kazakhstan": "kz",
    "azerbaijan": "az", "armenia": "am", "turkey": "tr", "cyprus": "cy",
    "luxembourg": "lu", "andorra": "ad", "monaco": "mc", "san marino": "sm",
    "malta": "mt", "moldova": "md", "north macedonia": "mk", "macedonia": "mk",
    "estonia": "ee", "latvia": "lv", "lithuania": "lt",

    # Middle East & Asia
    "qatar": "qa", "saudi arabia": "sa", "uae": "ae", "united arab emirates": "ae",
    "israel": "il", "japan": "jp", "korea": "kr", "south korea": "kr",
    "korea republic": "kr", "china": "cn", "hong kong": "hk", "macau": "mo", "macao": "mo",

    # Africa (expanded)
    "algeria": "dz", "angola": "ao", "benin": "bj", "botswana": "bw",
    "burkina faso": "bf", "burundi": "bi", "cabo verde": "cv", "cape verde": "cv",
    "cameroon": "cm", "central african republic": "cf", "car": "cf", "chad": "td",
    "comoros": "km", "congo": "cg", "republic of the congo": "cg",
    "congo brazzaville": "cg", "dr congo": "cd", "drc": "cd",
    ...
    "cote divoire": "ci", "cote d ivoire": "ci", "côte d’ivoire": "ci",
    "côte d'ivoire": "ci", "kenya": "ke", "lesotho": "ls", "liberia": "lr",
    ...
    "united republic of tanzania": "tz", "togo": "tg", "tunisia": "tn",
...
FB_ROLES = {
    ...
    "Attacking FB": {
        ...
        "Progressive runs per 90": 3, "Passes to penalty area per 90": 2,
        "xA per 90": 3
    },
    "Defensive FB": {
...
CM_ROLES = {
    ...
    "Defensive Midfielder": {
        ...
        "PAdj Interceptions": 3, "Aerial duels per 90": 0.5,
        "Aerial duels won, %": 1
    },
    "Goal Threat": {
        "Non-penalty goals per 90": 3, "xG per 90": 3,
        "Shots per 90": 1.5, "Touches in box per 90": 2
    },
    "Ball-Carrying": {
...
# Lower-is-better metrics => invert percentile
LOWER_BETTER = {
...
def compute_role_scores_for_row(row: pd.Series) -> Dict[str, int]:
    ...
    if g == "GK":
        roles = {k: weighted_role_score(row, w) for k, w in GK_ROLES.items()}
        return roles
...
def detect_minutes_col(df: pd.DataFrame) -> str:
    for c in ["Minutes played", "Minutes Played", "Minutes", "mins", "minutes", "Min"]:
        if c in df.columns:
            return c
    # fallback: look for any column containing "minute"
    for c in df.columns:
        if "minute" in c.lower() or c.lower().strip() in {"mins", "min"}:
            return c
...
def add_percentiles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # coerce numeric for all used metrics
    for m in used:
        if m in out.columns:
            out[m] = pd.to_numeric(out[m], errors="coerce")

    # rank pct within PosGroup (fairer)
    for m in used:
...
# add computed percentiles (from raw)
...
def _contract_year(row: pd.Series) -> str:
    for c in ("Contract expires", "Contract Expires", "Contract expiry", "Contract Expiry"):
        if c in row.index:
...
def _positions_html(pos: str) -> str:
    ...
    for t in tokens:
        if t not in seen:
            ...
            ordered.append(t)
    return "".join(
        f"<span class='postext' style='color:{_pro_chip_color(t)}'>{t}</span>" for t in ordered
    )
...
for i, row in df.iterrows():
    ...
    pills_html = (
        ...
        if roles_sorted
        else "<div class='row'><span class='chip'>No role scores</span></div>"
    )
